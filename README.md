...
默认值位于 `ReaderPreferences`：17sp 字号、1.75 倍行高、10dp 段距、2em 首行缩进、20dp 左右边距，以及 46dp/46dp 顶底预留。分页引擎还会为页眉页脚保留最小安全区，避免正文被系统栏或阅读状态遮挡；每章第一页会将章名作为正文标题块参与分页。内置护眼绿、米黄色、浅灰白、暖棕色、夜间和自定义主题。

设置面板中的字体、字号、行距、段距、缩进和边距会触发重新分页；背景与文字配色、页眉页脚开关、亮度、菜单、全屏点击翻页和翻页动画不会触发重新分页。

## 其他功能

- 书架、目录、阅读进度、全文搜索、书签和删除管理
- 点击、左右滑动及音量键翻页；可选“全屏点击翻页”会把左右两侧点击都改为下一页，中间窄区仍用于唤起菜单
...
2. 默认点击模式下，左/中/右点击应分别执行上一页、菜单、下一页。
3. 章内点击右侧 1 次应前进 1 页，快速连点右侧 2 次应最终前进 2 页。
4. 章内快速连点左侧 2 次应最终后退 2 页，左右滑动翻页应保持自然平移。
5. 打开设置面板，开启“全屏点击翻页”，确认设置面板与底部控制条之间没有额外空隙。
6. 隐藏菜单后，开启全屏点击模式时点击左侧 2 次应前进 2 页，点击右侧 2 次应继续前进 2 页；点击中间窄区域应唤起菜单。
7. 开启全屏点击模式后，左右滑动仍保持原来的上一页/下一页行为。
8. 在第 1 章最后一页快速连续点击右侧进入第 2 章，不得闪到第 3 章；进入第 2 章后应能继续点击或滑动到第 2 页。
9. 在第 2 章第一页点击或滑动返回上一章末页，不得白屏或乱页。
10. 音量键、自动阅读跨章只进入相邻章节；自动阅读在设置、目录、书签、更多设置打开时暂停，关闭后继续，书末自动停止。
11. 设置面板应贴在底部控制条上方，面板内部最后一行后无额外空洞；Android 16 真机需要重点复测。
